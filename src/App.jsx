...
export default function Portfolio() {
  ...
  return (
    <main className="site-shell">
      ...
      <nav className="nav-wrap"><a className="brand" href="#top" aria-label="Upendra Chauhan home">UC<span>.</span></a><div className="nav-links"><a href="#about">About</a><a href="#experience">Experience</a><a href="#projects">Projects</a><a href="#contact">Contact</a></div><a className="availability" href="mailto:up26445208@gmail.com"><span />Open to opportunities</a></nav>

      <section id="top" className="hero section-wrap"><motion.div className="hero-copy" {...reveal}><p className="eyebrow">Computer Science & AI undergraduate</p><h1>Building useful intelligence from <em>messy data.</em></h1><p className="hero-summary">I&apos;m Upendra Chauhan, an AI-focused developer turning ideas into reliable, data-driven applications across the machine learning lifecycle.</p><div className="hero-actions"><a className="button button-primary" href="#projects">Explore my work <ArrowUpRight /></a><a className="text-link" href="mailto:up26445208@gmail.com">Let&apos;s talk <ArrowUpRight /></a></div><div className="social-row">{links.map(({ label, href, icon: Icon }) => <a key={label} href={href} target={label !== "Email" ? "_blank" : undefined} rel="noreferrer"><Icon />{label}</a>)}</div></motion.div><motion.div className="hero-aside" initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.15 }}><div className="portrait-frame"><img src="/profile.png" alt="Upendra Chauhan" /></div><div className="hero-note"><strong>Currently</strong><span>Crafting AI systems that are clear, testable, and genuinely helpful.</span></div></motion.div></section>
      ...
      <section id="certifications" className="section-wrap certification-row"><div><p className="section-kicker">06 / Certifications</p><h2>Always <em>learning.</em></h2></div><div className="cert-list"><p><strong>Data Analytics</strong><span>Deloitte & Intellipaat</span></p><p><strong>Database Programming with SQL</strong><span>Oracle</span></p><p><strong>Data Science and Machine Learning</strong><span>CodeWithHarry</span></p></div></section>
